...
if { ${DESIGN_NAME} == "bp_tile_node" } {
  ...
  create_clock -period ${mem_clk_period_ps} -name ${mem_clk_name} [get_ports "mem_clk_i io_clk_i"]
  ...
  set io_input_pins [filter_collection [all_inputs] "name=~io*"]
  set core_input_pins [remove_from_collection [all_inputs] [concat $coh_input_pins $mem_input_pins $io_input_pins]]

  set coh_input_pins [filter_collection $coh_input_pins "name!~*clk*"]
  set mem_input_pins [filter_collection $mem_input_pins "name!~*clk*"]
  set io_input_pins [filter_collection $io_input_pins "name!~*clk*"]
  ...
  if { [sizeof $io_input_pins] > 0 }  { set_input_delay ${io_input_delay_ps} -clock ${mem_clk_name} ${io_input_pins} }

  # Reg2Out
  set coh_output_pins [filter_collection [all_outputs] "name=~coh*"]
  set mem_output_pins [filter_collection [all_outputs] "name=~mem*"]
  set io_output_pins [filter_collection [all_outputs] "name=~io*"]
  set core_output_pins [remove_from_collection [all_outputs] [concat ${coh_output_pins} ${mem_output_pins} ${io_output_pins}]]
  ...
  if { [sizeof $io_output_pins] > 0 }  { set_output_delay ${io_output_delay_ps} -clock ${mem_clk_name} ${io_output_pins} }
